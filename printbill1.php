...
// $distance_or_days = isset($_POST['distance_or_days']) ? $conn->real_escape_string($_POST['distance_or_days']) : 0;
// $fare = isset($_POST['hid_fare']) ? $conn->real_escape_string($_POST['hid_fare']) : 0;
...
$distance_or_days = isset($_POST['distance_or_days']) ? (int)$conn->real_escape_string($_POST['distance_or_days']) : 0;
$fare = isset($_POST['hid_fare']) ? (int)$conn->real_escape_string($_POST['hid_fare']) : 0;

if (!empty($distance_or_days) && !empty($fare)) {
    $total_amount = $distance_or_days * $fare;
} 
...
$sql0 = "SELECT rc.id, rc.rent_end_date, rc.charge_type, rc.rent_start_date, 
                 c.car_name, c.car_nameplate, rc.fare ,rc.total_amount,rc.no_of_days,rc.distance
...
if($charge_type == "days"){
    ...
    $sql1 = "UPDATE rentedcars SET car_return_date='$car_return_date', no_of_days='$duration', total_amount='$total_amount', return_status='$return_status' WHERE id = '$id' ";
}
